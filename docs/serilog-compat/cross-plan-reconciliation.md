...
### STEVE'S RULINGS (2026-05-29)

- **S-1 → `security` maps to `Warning`** (fail toward visibility). `notice`/`success`/`metric` → `Information`. True Herald level always preserved in a property (`LogEventValueProjector`, P1 Task 8). Pin the `security`→`Warning` mapping as a P8 G-GAP regression test.
- **S-3 → BUILD the migration shim.** A one-time read-side normalizer at the config-file-load boundary (`LoggingJsonSerializer`/`DefaultLoggingConfigurationMapper`) rewrites old level keys → new on load (saved in new keys thereafter). It is **separate** from P0 Task 9's wire-side loud-reject, which **stands unchanged**. Owner P8; P0 Task 9 references it so the two boundaries are not conflated.
