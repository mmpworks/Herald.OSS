...
// Pipeline-step toggles, filter slot, level cluster, and convenience
// pass-throughs to the per-collection Set classes. Renamed from
// QuickLogBuilder.With.cs (principal-review queue #19) after Rosanne's
// seam split lifted naming-policy, default sinks + loopback knobs, and
// the kernel-aware "fast" path into sibling partials. What remains is
// the largest cluster — pipeline-step toggles + filter slot + levels +
// convenience shortcuts — and stays together so future contributors do
// not have to chase a third-party predicate or a sampling filter across
// files. Notably the four writers of <c>_samplingFilter</c>
// (WithProjectionFilter, WithSampling / WithoutSampling,
// WithFilterExpression, WithCustomFilter) all live here as an atomic
// group; extracting a BuilderFilterSlot helper is deferred to a
// separate ticket.
//
// Collection operations delegate to their respective Set classes — callers
// can also use .Enrichers, .Channels, etc. directly.
public sealed partial class QuickLogBuilder
{
    ...
    // -- Network / integration sinks moved to QuickLogBuilder.NetworkSinks.cs --
