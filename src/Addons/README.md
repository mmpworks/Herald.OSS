...
The "Edition" column below carries the upstream Herald.Core gating intent for each addon (Community / Pro / Enterprise). Herald.OSS itself ships every listed addon — there is no runtime gate in the OSS distribution, and registration succeeds regardless of the value in that column. Downstream commercial wrappers may layer their own gate on top.
...
| Addon | Tier (informational) | Threading | Tests / Benchmarks |
|---|---|---|---|
